...
# 파라미터 벡터 (크기 10): 
# [Fc+ , Fs+ , Vs+ , delta+ , s2+ , Fc- , Fs- , Vs- , delta- , s2-]
p = opti.variable(10)

# Stribeck 마찰 모델 정의 (v에 따라 조건부로 분기)
def stribeck_friction(v, p):
    # v >= 0 인 경우
    expr_pos = p[0] + (p[1] - p[0]) * ca.exp(-((ca.fabs(v) / p[2]) ** p[3])) + p[4] * v
    # v < 0 인 경우
    expr_neg = -p[5] - (p[6] - p[5]) * ca.exp(-((ca.fabs(v) / p[7]) ** p[8])) + p[9] * v
...
opti.subject_to(p[7] <= 10)

# 초기 파라미터 설정 (C++ 코드의 초기값 사용)
p0 = [11,12, 0.000200, 4.000000, 20.797302,
      11,12, 0.000200, 4.000000, 20.797302]
