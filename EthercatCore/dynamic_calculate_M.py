...
# -------------------------------
# Load dynamic data
# preslide_data = pd.read_csv('/home/user/release/friction_data/RT_test/RT-data-4.csv')
...
# Load slide (정적) data
# slide_data = pd.read_csv('/home/user/release/friction_data/RT_test/RT-data-3.csv')
...
for t in range(T_dyn):
    if velocity[t] > 0:
        s_v_array[t] = Fcp + (Fsp - Fcp) * np.exp(- (velocity[t] / Vsp) ** 2)
    else:
        s_v_array[t] = Fcn + (Fsn - Fcn) * np.exp(- (abs(velocity[t]) / Vsn) ** 2) 
...
for t in range(T_slide):
    if slide_velocity[t] > 0:
        s_v_array_slide[t] = Fcp + (Fsp - Fcp) * np.exp(- (slide_velocity[t] / Vsp) ** 2)
    else:
        s_v_array_slide[t] = Fcn + (Fsn - Fcn) * np.exp(- (abs(slide_velocity[t]) / Vsn) ** 2) 
