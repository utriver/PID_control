...
// }
////////////////////////////////static mode////////////////////////////////
void FrictionDataLogger::write_rt_buffer0(int filenum, double &percent_ready)
{
...
void FrictionDataLogger::write_rt_buffer1(int filenum, double &percent_ready)
{
    ...
    for (int i = 0; i < CTRL_BUFF_SIZE1; i++)
    {
         _rtLogger_save[i].time = _loggingBuff_ctrl1[i].time;
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].q[j] = _loggingBuff_ctrl1[i].q[j];
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].qdes[j] = _loggingBuff_ctrl1[i].qdes[j];
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].qdot[j] = _loggingBuff_ctrl1[i].qdot[j];
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].qdotdes[j] = _loggingBuff_ctrl1[i].qdotdes[j];
        // for (int j = 0; j < NUM_AXIS; j++)
        //     _rtLogger_save[i].qddot[j] = _loggingBuff_ctrl[i].qddot[j];
        // for (int j = 0; j < NUM_AXIS; j++)
        //     _rtLogger_save[i].ActualVel[j] = _loggingBuff_ctrl[i].ActualVel[j];
        ...
        //     _rtLogger_save[i].ActualPOS[j] = _loggingBuff_ctrl[i].ActualPOS[j];
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].friction_torque[j] = _loggingBuff_ctrl1[i].friction_torque[j];
        for (int j = 0; j < NUM_AXIS; j++)
            _rtLogger_save[i].coretor[j] = _loggingBuff_ctrl1[i].coretor[j];
        // for (int j = 0; j < NUM_AXIS; j++)
        //     _rtLogger_save[i].sensortor[j] = _loggingBuff_ctrl[i].sensortor[j];
        // for (int j = 0; j < NUM_AXIS; j++)
        //     _rtLogger_save[i].ActualTor[j] = _loggingBuff_ctrl[i].ActualTor[j];
